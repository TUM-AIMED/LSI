...
def computeKL_from_blocks(DEVICE, model1, model2, train_loader, criterion, weight_reg):
    blocks_model1, means1, blocks_model2, means2 = _compute_block_diag(DEVICE, model1, model2, train_loader, criterion)
    # blocks_model2, means2 = _compute_block_diag(DEVICE, model2, train_loader, criterion)
    blocks_model1_w_prior = _add_prior_to_block(weight_reg, blocks_model1)
    blocks_model2_w_prior = _add_prior_to_block(weight_reg, blocks_model2)
    kl1, kl2 = _computeblockKL(means1, means2, blocks_model1_w_prior, blocks_model2_w_prior)
    print(f"kl1 {kl1} kl2 {kl2}")
    return kl1, kl2
...
if __name__ == "__main__":
    ...
    params["model"]["dataset_name"] = "cifar100"
    ...
    params["Paths"]["feld_path"] = "/vol/aimspace/users/kaiserj/Individual_Privacy_Accounting/Feld_Score/cifar100_infl_matrix.npz"

    print("--------------------------")
    print("Load Memorization scores")
    print("--------------------------")


    loaded_data = np.load(params["Paths"]["feld_path"])
    mem_scores = loaded_data["tr_mem"]
    tr_labels = loaded_data["tr_labels"]
    keep_indices_w_labels = [idx for idx in range(len(tr_labels)) if tr_labels[idx] < 30]
    mem_scores_cut = np.array([mem_scores[i] for i in keep_indices_w_labels])
    tr_labels = np.array([tr_labels[i] for i in keep_indices_w_labels])

    n = 500
    sorted_mem_index_array_idx = np.argsort(mem_scores_cut) 
    sorted_mem_index_array = [keep_indices_w_labels[i] for i in sorted_mem_index_array_idx]
    # sorted_mem_index_array = [sorted_mem for sorted_mem in sorted_mem_index_array if sorted_mem in keep_indices_w_labels]
    sorted_mem_array = mem_scores[sorted_mem_index_array] 
    highest_values = sorted_mem_array[-n : ] 
    highest_values_index = sorted_mem_index_array[-n : ]
    lowest_values = sorted_mem_array[:n] 
    lowest_values_index = sorted_mem_index_array[:n]
    split = 1.37 # split=2 takes the middle of the array, but results in values ~0.05 for mem
    mid_values = sorted_mem_array[int(len(sorted_mem_array)/(split) - n/2):int(len(sorted_mem_array)/(split) + n/2)] 
    mid_values_index = sorted_mem_index_array[int(len(sorted_mem_array)/(split) - n/2):int(len(sorted_mem_array)/(split) + n/2)]

    keep_indices = [*highest_values_index, *mid_values_index ,*lowest_values_index]
    keep_indices.sort()

    mem_scores_test = [mem_scores[index] for index in keep_indices]
    ...
    data_set_test.reduce_to_active_class([*range(30)])
