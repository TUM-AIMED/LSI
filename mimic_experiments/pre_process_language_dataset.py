...
test_labels = torch.squeeze(torch.randint(0, 2, (num_reviews, 1)))
encoded_reviews_test = torch.zeros((num_reviews, 768))  # BERT's CLS embedding is 768-dimensional
for start_idx in tqdm(range(0, num_reviews, batch_size)):
    end_idx = min(start_idx + batch_size, num_reviews)
...
base_path = "/vol/aimspace/users/kaiserj/Datasets/Datasets_compressed_by_bert_headless/LoremIpsum"
...
# torch.save(train_labels.cpu(), os.path.join(base_path, 'train_labels.pt'))
torch.save(test_labels.cpu(), os.path.join(base_path, 'test_labels.pt'))


# Print the shape of the encoded reviews
# print(encoded_reviews_train.shape)

# Save the encoded reviews to a file
# np.save('encoded_imdb_reviews.npy', encoded_reviews_train)
