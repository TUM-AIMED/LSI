...
def main():
    ...
    memorization, influence = get_feld_scores(feld_path, idx)
    plt.hist(memorization, bins="auto", edgecolor='black')  # Adjust the number of bins as needed
    plt.title('Histogram')
    plt.xlabel('memorization score')
    plt.ylabel('Frequency')
    plt.savefig('z_mem_hist.jpg')


    tar_label = None
    plot = "test"

    file_name = "z_memorization_vs_grad_norm"
    xcompare = averages
    ycompare = memorization
    zcompare = None
    x_label = "sum_grad_norm"
    y_label = "memorization"
    show_images = False
    animate = True
