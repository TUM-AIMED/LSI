...
if __name__ == "__main__":
    ...
    parser.add_argument("--name_ext", type=str, default="mult_model")
    ...
    path_name = "/vol/aimspace/users/kaiserj/Individual_Privacy_Accounting/results_multi_model"
    ...
    for seed in range(N_SEEDS):
        ...
        for i in tqdm(range(epochs)):
            ...
            optimizer.step()
            if i % 100 == 0:
                init_models.append(deepcopy(model))
        means1, precs1 = [], []
        ...
        for i in tqdm(range(N_REMOVE1, N_REMOVE2)):
            ...
            for j in range(epochs):
                ...
                optimizer.step()
                if j % 100 == 0:
                    llo_models.append(deepcopy(model))
            kl1_all = []
            for j, model in enumerate(llo_models):
                if full:
                    mean2, prec2 = get_mean_and_prec(X_train_rem.cpu(), y_train_rem.cpu(), model, mode="full")
                    kl1, square_diff1  = _computeKL_from_full(means1[j], mean2, precs1[j], prec2)
                else:
                    mean2, prec2 = get_mean_and_prec(X_train_rem.cpu(), y_train_rem.cpu(), model)
                    kl1, square_diff1 = _computeKL(means1[j], mean2, precs1[j], prec2)
                kl1_all.append(kl1)            
            kl_seed.append(kl1_all)
