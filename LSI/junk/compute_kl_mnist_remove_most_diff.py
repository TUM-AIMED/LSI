...
def train(
    ...
    if params["Full_Batch"]:
        ...
        data, target = data.to(DEVICE), target.to(DEVICE)
    # print(f"Transfering data took {time.time() - start_time}")
    start_time = time.time()
    for epoch in range(params["training"]["num_epochs"]):
        ...
        train_loss, train_accuracy = normal_train_step(params,
               ...
               idx=idx)
        model.eval()
        if epoch % params["testing"]["test_every"] == 0 or epoch == params["training"]["num_epochs"] - 1:
            val_loss, val_accuracy = normal_val_step(params,
                ...
                criterion, 
                test_loader)
    # print(f"Real Training took {time.time() - start_time}")
...
def train_with_params(
    ...
    # print(f"Setting up model took {time.time() - start_time}")
    start_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_seeds", type=int, default=9, help="Value for lerining_rate (optional)")
    parser.add_argument("--n_rem", type=int, default=750, help="Value for lerining_rate (optional)")
    ...
    parser.add_argument("--subset", type=int, default=7500, help="Value for lerining_rate (optional)")
    parser.add_argument("--lr", type=float, default=1e-1, help="Value for lerining_rate (optional)")

    args = parser.parse_args()
    N_REMOVE_EACH_ITER = args.n_rem
    print("--------------------------", flush=True)
    print("--------------------------", flush=True)
    ...
    params["testing"] = {}
    params["testing"]["test_every"] = 10000 # params["training"]["num_epochs_init"] + 1
    params["Paths"] = {}
    ...
    most_difficult_idx = []
    for seed in tqdm(range(N_SUBSET_ORDERING)):
        print("--------------------------", flush=True)
        ...
        for idx in most_difficult_idx:
            data_set.remove_index_from_data(idx)
        ...
        for rem_idx in tqdm(range(N_REMOVE)):
            ...
            # print(f"Setting up data took {time.time() - start_time}")
            start_time = time.time()
            ...
            # print(f"KL took {time.time() - start_time}")
            start_time = time.time()
            ...
            correct_val_list = []
            for _, (data, target, idx, _) in enumerate(test_loader):
                data, target = data.to(DEVICE), target.to(DEVICE)
                output = model_rem(data)
                _, predicted = torch.max(output.data, 1)
                correct_val_list.append((predicted == target).cpu().numpy())
            correct_val[seed][true_rm_idx] = correct_val_list
            # print(f"Getting correct labels took {time.time() - start_time}")
            start_time = time.time()
        ...
        kl1_diag = []
        init_idx = []
        test = resultskl1_diag[seed]
        for idx, kl1 in resultskl1_diag[seed].items():
            kl1_diag.append(kl1)
            init_idx.append(idx)
        combined_data = list(zip(kl1_diag, init_idx))
        sorted_data = sorted(combined_data, key=lambda x: np.median(x[0]))
        kl1_diag, init_idx = zip(*sorted_data)
        if N_REMOVE_EACH_ITER == 0:
            most_difficult_idx = []
        else:
            most_difficult_idx = list(init_idx[-N_REMOVE_EACH_ITER:])
            print(init_idx[-1])
        difficult_idx[seed] = most_difficult_idx


    if args.name != None:
        params["Paths"]["final_save_path"] = "/vol/aimspace/users/kaiserj/Individual_Privacy_Accounting/results_kl_indiv_diff_final/results_" + str(args.name)
    else:
        name_str = f"{params['model']['dataset_name']}_{params['model']['model']}_{N_SUBSET_ORDERING}_{N_REMOVE_EACH_ITER}_{args.epochs}_{args.subset}_{train_accuracy:.4f}_{params['training']['learning_rate']}{args.name_ext}"
        params["Paths"]["final_save_path"] = "/vol/aimspace/users/kaiserj/Individual_Privacy_Accounting/results_kl_indiv_diff_final/results_" + name_str
