...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=2000, help="Value for lerining_rate (optional)")
    ...
    params["training"]["learning_rate"] = 3e-1# 0.002 mlp #3e-03 # -3 for mnist
    ...
    if args.name != None:
        ...
        name_str = params["model"]["dataset_name"] + "_" + params["model"]["model"] + "_" + str(N_SEEDS) + "_" + str(args.clip) + "_" + str(args.corrupt)  + "_" + str(args.epochs) + "_" + str(args.subset) + str(args.name_ext)
        params["Paths"]["final_save_path"] = "/vol/aimspace/users/kaiserj/Individual_Privacy_Accounting/results_idp/results_" + name_str
    ...
    for i in tqdm(range(N_SEEDS)):
        params["model"]["seed"] = i
        _, _, grad_norms = train_with_params(
            params,
            train_loader,
            test_loader
        )
        grad_norms_all.append(grad_norms.cpu())
