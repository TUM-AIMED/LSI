...
def normal_val_step(params,
    ...
    optimizer.zero_grad()
    torch.cuda.empty_cache()
    print(f"testing accuracy {accuracy:.4f}, epoch took {time.time() - start_time:.4f}")
    return np.mean(np.array(loss_list)), accuracy
...
def train_with_params(
    ...
    if params["model"]["model"] == "mlp" or params["model"]["model"] == "small_mlp":
        ...
        model = model_class(len(train_X_example), N_CLASSES, pretrained=True)
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [300, 500, 750, 1000], gamma=0.5)
...
if __name__ == "__main__":
    ...
    params["model"]["model"] = "resnet18"
    params["model"]["dataset_name"] = "cifar10"
    params["model"]["name_base"] = "laplace_cifar10_"
    params["training"] = {}
    params["training"]["batch_size"] = 256
    params["training"]["learning_rate"] = 1e-03# 0.002 mlp #3e-03 # -3 for mnist
    params["training"]["l2_regularizer"] = 1e-05
    params["training"]["num_epochs_init"] = 300
    params["testing"] = {}
    params["testing"]["test_every"] = 10 # params["training"]["num_epochs_init"] - 1
    params["Paths"] = {}
    ...
    for seed in tqdm(range(N_SEEDS)):
        ...
        for rem_idx in range(N_REMOVE):
            ...
            if "kron" in representation:
                ...
                kl1_1, kl2_1, mean_diff_s, mean_diff_m = computeKL(DEVICE, backend_class, "kron", model_rem, model_all, train_loader_0, params["training"]["l2_regularizer"], subset_of_weights="last_layer") 
            ...
            if "diag" in representation:
                ...
                kl1_2, kl2_2, mean_diff_s, mean_diff_m = computeKL(DEVICE, backend_class, "diag", model_rem, model_all, train_loader_0, params["training"]["l2_regularizer"], subset_of_weights="last_layer") 
            ...
            if "full" in representation:
                ...
                kl1_3, kl2_3, mean_diff_s, mean_diff_m = computeKL(DEVICE, backend_class, "full", model_rem, model_all, train_loader_0, params["training"]["l2_regularizer"], subset_of_weights="last_layer") 
