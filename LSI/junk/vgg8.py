...
class CustomVGG8(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(CustomVGG8, self).__init__()
        ...
        self.features = torch.nn.Sequential(self.conv1, self.relu1, self.max_pool,
                                            self.conv2, self.relu2, self.max_pool,
                                            self.conv3, self.relu3, self.max_pool,
                                            self.conv4, self.relu4,
                                            self.conv5, self.relu5, self.avg_pool,
                                            self.flatten, 
                                            self.fc1, self.relu3, 
                                            self.fc2 )
