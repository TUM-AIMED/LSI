...
def custum_hvp(f, primals, tangents, strict=False):
    primals = listit(primals)
    primals[1][1] = primals[1][1].double()
    primals = tupleit(primals)
    tangents = listit(tangents)
    tangents[1][1] = tangents[1][1].double()
    tangents = tupleit(tangents)
