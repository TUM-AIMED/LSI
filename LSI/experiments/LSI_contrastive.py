...
def download_coco_dataset():
    ...
    dataset_dir = "/vol/aimspace/users/kaiserj/Datasets/Datasets_Raw/coco"
    ...
    if not os.path.exists(annotations_dir):
        ...
        annotations_zip_path = os.path.join(dataset_dir, "annotations_trainval2017.zip")
    ...
    if not os.path.exists(train_images_dir):
        ...
        train_images_zip_path = os.path.join(dataset_dir, "train2017.zip")
        ...
        with zipfile.ZipFile(train_images_zip_path, "r") as zip_ref:
            zip_ref.extractall(dataset_dir)
        os.remove(train_images_zip_path)
        print("COCO train images downloaded and extracted.")

    return annotations_dir, train_images_dir
...
def precompute_embeddings(folder, resnet, bert, tokenizer):
    if os.path.exists(folder):
        ...
        annotations_dir, image_dir = download_coco_dataset()

        # Load the COCO dataset
        coco_cap = COCO(os.path.join(annotations_dir, 'captions_train2017.json'))
        coco_ann = COCO(os.path.join(annotations_dir, 'instances_train2017.json'))
