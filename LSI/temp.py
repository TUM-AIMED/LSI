...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=100, help="Value for lerining_rate (optional)") # 150 for Resnet, 700 for CNN, 600 for MLP
    ...
    parser.add_argument("--range2", type=int, default=2, help="Value for lerining_rate (optional)")
    ...
    file_name = "full_vs_lastlayer_SmallCNN"
    ...
    data_set.reduce_to_active_class([0, 1])
    ...
    for i in tqdm(range(N_REMOVE1, N_REMOVE2)):
        X_train_rem = X_train # torch.cat([X_train[0:i], X_train[i+1:]])
        y_train_rem = y_train # torch.cat([y_train[0:i], y_train[i+1:]])
    ...
    model = model_class_proxy(n_classes)
    backup_model = deepcopy(model)
    model = model.to(DEVICE)
    optimizer = torch.optim.SGD(
        model.parameters(),
    ...
    for i in tqdm(range(epochs)):
        optimizer.zero_grad()
        output = model(compressed_X_train)
        pred = torch.argmax(output, dim=1)
        accuracy = torch.sum(pred == y_train)/y_train.shape[0]
        loss = criterion(output, y_train)
        loss.backward()
    ...
    if args.laptype == "full":
        mean1, prec1 = get_mean_and_prec(compressed_X_train.cpu(), y_train.cpu(), model, mode="full")
    elif args.laptype =="diag":
        mean1, prec1 = get_mean_and_prec(compressed_X_train.cpu(), y_train.cpu(), model, mode="diag")
    elif args.laptype =="kfac":
        mean1, prec1 = get_mean_and_prec(compressed_X_train.cpu(), y_train.cpu(), model, mode="kfac")
    else:
        raise Exception("Not implemented")
    ...
    for i in tqdm(range(N_REMOVE1, N_REMOVE2)):
        ...
        for j in range(epochs):
            optimizer.zero_grad()
            output = model(compressed_X_train_rem)
            loss = criterion(output, y_train_rem)
            loss.backward()
            optimizer.step()
        if args.laptype == "full":
            mean2, prec2 = get_mean_and_prec(compressed_X_train_rem.cpu(), y_train_rem.cpu(), model, mode="full")
            kl1_proxy, square_diff1  = _computeKL_from_full(mean1, mean2, prec1, prec2)
        elif args.laptype =="diag":
            mean2, prec2 = get_mean_and_prec(compressed_X_train_rem.cpu(), y_train_rem.cpu(), model, mode="diag")
            kl1_proxy, square_diff1 = _computeKL(mean1, mean2, prec1, prec2)
        elif args.laptype =="kfac":
            mean2, prec2 = get_mean_and_prec(compressed_X_train_rem.cpu(), y_train_rem.cpu(), model, mode="kfac")
            kl1_proxy, square_diff1 = _computeKL_from_kron(mean1, mean2, prec1, prec2)
        else:
            raise Exception("Not implemented")
        

        print(f"KL divergence {kl1_proxy}")
